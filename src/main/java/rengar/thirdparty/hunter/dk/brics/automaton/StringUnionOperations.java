...
final public class StringUnionOperations {
	 ...
	 */
	private static rengar.thirdparty.hunter.dk.brics.automaton.State convert(State s,
                                                                             IdentityHashMap<State, rengar.thirdparty.hunter.dk.brics.automaton.State> visited) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}

		rengar.thirdparty.hunter.dk.brics.automaton.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new rengar.thirdparty.hunter.dk.brics.automaton.State();
	 ...
	 */
	public static rengar.thirdparty.hunter.dk.brics.automaton.State build(CharSequence[] input) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
		...
		return convert(builder.complete(), new IdentityHashMap<State, rengar.thirdparty.hunter.dk.brics.automaton.State>());
	}
