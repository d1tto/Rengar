package rengar.thirdparty.hunter.cn.ac.ios.Utils;

import rengar.thirdparty.hunter.cn.ac.ios.TreeNode.TreeNode;
import rengar.thirdparty.hunter.cn.ac.ios.TreeNode.Utils;

import java.util.*;
import java.util.regex.Pattern;

import static rengar.thirdparty.hunter.cn.ac.ios.TreeNode.Utils.createReDoSTree;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.FlagsUtils.isSpecialCharacter;
...
public class RegexUtils {
     ...
     * @return a-z,\n,\r
     */
    private static Set<String> simplifyLetters(Set<String> set, Constant.SimplyLevel level) {
        ...
        newSet.addAll(BracketUtils.getLettersSimply(characterSet));
    ...
    public static boolean isQuantifierNode(TreeNode treeNode) {
        String data = treeNode.getData();
        if (!Pattern.matches(Constant.ALL_COUNTING, data)) return false;
    ...
    public static boolean isUnusualBackReferencesNode1(TreeNode treeNode) {
        String data = treeNode.getData();
        if (Constant.PATTERN_BACKSLASH_G_N_REGEX_1.matcher(data).matches()) {    // \gn
            return true;
        }
        return false;
    }
    ...
    public static boolean isUnusualBackReferencesNode2(TreeNode treeNode) {
        String data = treeNode.getData();
        if (Constant.PATTERN_BACKSLASH_G_N_REGEX_2.matcher(data).matches()) { //  \g{n}
            return true;
        }
        return false;
    }
    ...
    public static boolean isUnusualBackReferencesNode3(TreeNode treeNode) {
        String data = treeNode.getData();
        if (Constant.PATTERN_BACKSLASH_G_N_REGEX_3.matcher(data).matches()) { // \g{-n}
            return true;
        }
        return false;
    }
    ...
    public static boolean isCanEmptyNode(TreeNode node) {
        ...
        if (isGroupNode(node)) {
            ...
            if (isQuantifierNode(node)) {
                return true;
            } else if (FlagsUtils.isSpecialCharacter(node)) {
                return true;
            }
            return false;
        } else {
    ...
    public static boolean isOrNode(TreeNode node) {
        ...
        if (BracketUtils.isBracketsNode(node) ||
                BracketUtils.isInBrackets(node) ||
                isNegateNode(node) ||
                BracketUtils.isInNegateNode(node) ||
...
//        return true;
    ...
    public static boolean isCountingNode(TreeNode node) {
        ...
        if (node.getChildCount() == 2) {
            return Pattern.matches(Constant.COUNTING, node.getChild(1).getData());
        }
        return false;
    }
    ...
    public static boolean isGeneralizedCountingNode(TreeNode node) {
        ...
        if (node.getChildCount() == 2) {
            if (Pattern.matches(Constant.COUNTING, node.getChild(1).getData())) {
                return true;
            }
...
//            }
    ...
    public static boolean isEqualCountingNode(TreeNode node) {
        ...
        if (node.getChildCount() == 2) {
            if (Pattern.matches(Constant.EQUAL_COUNTING, node.getChild(1).getData())) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isSetRepeatNode(TreeNode node) {
        ...
        if (node.getChildCount() == 2 && ("*".equals(node.getChild(1).getData()) || "+".equals(node.getChild(1).getData()))) {
            ...
            boolean result = isSetNode(node) || BracketUtils.isBracketsNode(node) || isNegateNode(node);
            if (result) {
                return true;
            } else {
                try {
                    return BracketUtils.isBracketsNode(node) || isNegateNode(node) || isSetNode(removeOption(node));
    ...
    public static TreeNode removeOption(TreeNode node) throws InterruptedException {
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            ...
            if (isSetNode(node) || BracketUtils.isBracketsNode(node)) {
        ...
        return Utils.createReDoSTree(stringBuilder.toString());
    }
    ...
    public static boolean isZeroWidthAssertionNode(TreeNode node) {
        ...
        if (node.isLeaf() && !BracketUtils.isInBrackets(node) && !BracketUtils.isInNegateNode(node)) {
    ...
    public static String getLookAroundRegex(TreeNode node) {
        ...
        if (assertType == Constant.AssertType.NEGATIVE_AHEAD) {
        ...
        } else if (assertType == Constant.AssertType.POSITIVE_AHEAD) {
            if (node.getChildCount() == 5) {
                return node.getChild(3).getData();
            } else if (node.getChildCount() == 4) {
                return "";
     ...
     * @return
     */
    public static Constant.AssertType getAssertType(TreeNode node) {
        if (isLookAroundNode(node)) {
            if ((node.getChildCount() == 4 || node.getChildCount() == 5) && node.getData().startsWith("(?!")) {
                if ("(".equals(node.getChild(0).getData()) && "?".equals(node.getChild(1).getData()) && "!".equals(node.getChild(2).getData())) {
                    return Constant.AssertType.NEGATIVE_AHEAD;
                }
            }
            if ((node.getChildCount() == 5 || node.getChildCount() == 6) && node.getData().startsWith("(?<!")) {
                if ("(".equals(node.getChild(0).getData()) && "?".equals(node.getChild(1).getData()) && "<".equals(node.getChild(2).getData()) && "!".equals(node.getChild(3).getData())) {
                    return Constant.AssertType.NEGATIVE_BEHIND;
                }
            }
            if ((node.getChildCount() == 4 || node.getChildCount() == 5) && node.getData().startsWith("(?=")) {
                if ("(".equals(node.getChild(0).getData()) && "?".equals(node.getChild(1).getData()) && "=".equals(node.getChild(2).getData())) {
                    return Constant.AssertType.POSITIVE_AHEAD;
                }
            }
            if ((node.getChildCount() == 5 || node.getChildCount() == 6) && node.getData().startsWith("(?<=")) {
                if ("(".equals(node.getChild(0).getData()) && "?".equals(node.getChild(1).getData()) && "<".equals(node.getChild(2).getData()) && "=".equals(node.getChild(3).getData())) {
                    return Constant.AssertType.POSITIVE_BEHIND;
                }
            }
        } else if (node.isLeaf() && !BracketUtils.isInBrackets(node) && !BracketUtils.isInNegateNode(node)) { // fix bug 不能在[...]或[^...]中
            switch (node.getData()) {
                case "\\b":
                    return Constant.AssertType.WORD_BOUNDARY_ASSERT;
                case "\\B":
                    return Constant.AssertType.NON_WORD_BOUNDARY_ASSERT;
                case "^":
                    return Constant.AssertType.START_ASSERT;
                case "$":
                    return Constant.AssertType.END_ASSERT;
                case "\\A": // 新增
                    return Constant.AssertType.START_ASSERT;
                case "\\Z": // 新增
                    return Constant.AssertType.END_ASSERT;
        ...
        return Constant.AssertType.NO_ASSERT;
    }
...
//        }
    ...
    public static boolean isNegateSingleNode(TreeNode node) {
        ...
        if (node.isLeaf()) {
            return Constant.COMPLEMENTARY_SET_SYMBOL.containsKey(node.getData());
        }
        return false;
    }
...
//                    break;
    ...
    public static String getMatchBracketNode(TreeNode node) {
        ...
        if ("\\w".equals(set)) {
            ...
            s.append("a");
        } else if (BracketUtils.isCollectionSymbol(set)) {
            String ch = "";
    ...
    public static String getMatchLetter(String letter, List<String> conflictList) {
        ...
        switch (letter) {
            case "\\w":
                for (String str : Constant.w_MATCH_CHARACTER_LIST) {
                ...
                break;
            case "\\d":
                for (String str : Constant.d_MATCH_CHARACTER_LIST) {
                    if (conflictList.contains(str)) {
                        returnLetter = str;
                        break;
                    }
                ...
                break;
            case "\\s":
                for (String str : Constant.s_MATCH_CHARACTER_LIST) {
                    if (conflictList.contains(str)) {
                        returnLetter = str;
                        break;
                    }
            ...
            default:
                if (conflictList.contains(letter)) {
                    ...
                    if (BracketUtils.isSpecialCharacterBracket(letter.charAt(0)) && conflictList.contains("\\" + letter)) {
                        returnLetter = letter;
                    } else {
    ...
    public static String getPureTextInOrNode(TreeNode orNode) throws InterruptedException {
        ...
        // (((?=c)|(?=d)).*|a) -> (().*|a)
        // 需要调用去空
        TreeNode resultTree = Utils.createReDoSTree(result);
    ...
    public static void main(String[] args) throws InterruptedException {
        String re = "(|.*(|a|))+";
        TreeNode treeNode = Utils.createReDoSTree(re);
