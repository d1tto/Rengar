package rengar.thirdparty.hunter.cn.ac.ios;

import rengar.thirdparty.hunter.cn.ac.ios.Patterns.SLQ.PatternSLQUtils;
import rengar.thirdparty.hunter.cn.ac.ios.TreeNode.TreeNode;
import rengar.thirdparty.hunter.cn.ac.ios.Patterns.EOA.PatternEOAUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Patterns.EOD.PatternEODUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Patterns.NQ.PatternNQUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Patterns.POA.PatternPOAUtils;
...
public class ReDoSMain {
    ...
    public static ReDoSBean getJava(ReDoSBean bean, String model) {
        RegexBean divideRegexByFlagsBean = FlagsUtils.divideRegexByFlags(bean.getRegex());
...
//                regex = replaceLocalFlagGM(regex);
    ...
    public static ReDoSBean getPython(ReDoSBean bean, String model) {
        bean.setRegex(FlagsUtils.divideRegexByFlags(bean.getRegex()).getRegex());
    ...
    public static ReDoSBean getJS(ReDoSBean bean, String model) {
        ...
        List<String> list = new ArrayList<>();
        String regex = FlagsUtils.divideRegexByFlags(bean.getRegex()).getRegex();
        String flags = FlagsUtils.divideRegexByFlags(bean.getRegex()).getFlags();
        list.add(regex);
        list.add(flags);
        list.add(model);
        for (int i = 0; i < bean.getAttackBeanList().size(); i++) {
    ...
    public static boolean mustBeNoReDoS(String regex) {
        Matcher matcher = Pattern.compile(Constant.EXTENDED_COUNTING).matcher(regex);
        return !matcher.find();
    }
}
