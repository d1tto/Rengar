package rengar.thirdparty.hunter.cn.ac.ios.Patterns.SLQ;
...
import static rengar.thirdparty.hunter.cn.ac.ios.Patterns.SLQ.SLQUtils.*;
...
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.NegateUtils.refactorAssertPattern;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.RegexUtils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.RegexUtils.isCanEmptyNode;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.SpecialStringUtils.getSpecialSuffixStringWithCountingAndEndLine;
...
//            node = node.getNextNode();
    ...
    private static ArrayList<AttackBean> getSLQConditionSp(TreeNode root) throws InterruptedException {
        ...
        for (TreeNode leaf: leafNodes) {
            if (BracketUtils.isInBrackets(leaf) || BracketUtils.isInNegateNode(leaf)) continue;
            ...
            if (isOrNode(regexTree)) {
                ...
                for (int i = 0; i < regexTree.getChildCount() - 2; i += 2) {
                    for (int j = i + 2; j < regexTree.getChildCount(); j++) {
                        ...
                        String infix = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1, null, 1);
...
//        deleteZeroWidthAssertion(regexTree);
    ...
    private static String getMatchString(String regex) throws InterruptedException {
        ...
        List<String> innerTrans = GenMatchStringUtils.getTranslateRegexForAssertionsList(node);
