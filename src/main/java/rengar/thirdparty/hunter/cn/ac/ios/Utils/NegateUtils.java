...
public class NegateUtils {
    ...
    public static Set<String> getSetFromNegateNode(TreeNode treeNode) {
        ...
        for (int i = 2; i < treeNode.getChildCount() - 1; i++) {
            ...
            if ("\\w".equals(str)) {
                set.addAll(Arrays.asList(Constant.w_MATCH_CHARACTER_LIST));
            } else if ("\\d".equals(str)) {
                set.addAll(Arrays.asList(Constant.d_MATCH_CHARACTER_LIST));
            } else if ("\\s".equals(str)) {
                set.addAll(Arrays.asList(Constant.s_MATCH_CHARACTER_LIST));
        ...
        for (String str : set) {
            if (str.length() == 1 && BracketUtils.isSpecialCharacterBracket(str.charAt(0))) {
                newSet.add("\\" + str);
            } else {
                newSet.add(str);
            }
    ...
    public static TreeNode getLastNegateNode(TreeNode treeNode) {
        ...
        while (treeNode != null && !treeNode.getChildList().isEmpty()) {
            ...
            if (getAssertType(treeNode) != Constant.AssertType.NO_ASSERT && getAssertType(treeNode)  != null) {
     ...
     * @return
     */
    public static void removeNegateSymbol(TreeNode treeNode, Constant.SimplyLevel level) throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            ...
            if (isNegateNode(node)) {
                ...
                if (set.contains("\\W")) {
                    temp.clear();
                    temp.addAll(Constant.COMPLEMENTARY_SET_SYMBOL.get("\\W"));
                }
                if (set.contains("\\D")) {
                    temp.clear();
                    temp.addAll(Constant.COMPLEMENTARY_SET_SYMBOL.get("\\D"));
                }
                if (set.contains("\\S")) {
                    temp.clear();
                    temp.addAll(Constant.COMPLEMENTARY_SET_SYMBOL.get("\\S"));
                }
                temp.removeAll(set);
                temp = BracketUtils.simplifyLetters(temp, level);
            ...
            } else if (isNegateSingleNode(node) && !BracketUtils.isInBrackets(node) && !BracketUtils.isInNegateNode(node)) {
                Set<String> set = new HashSet<>(Constant.COMPLEMENTARY_SET_SYMBOL.get(node.getData()));
                Set<String> temp = new LinkedHashSet<>(letterSet);
                // 加上特殊符号
                temp.add("!");
                temp.removeAll(set);
    ...
    public static TreeNode removeGroup2(TreeNode treeNode) throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            ...
            if (isGroupNode(node)) {
                ...
                if (BracketUtils.isBracketsNode(cur)) {
...
//                    node.updateTreeByModifyNode(cur.getParent().getParent().getData());
    ...
    public static TreeNode refactorToDot(TreeNode treeNode) throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            TreeNode node = stack.pop();
            if (BracketUtils.isBracketsNode(node) || "\\w".equals(node.getData()) || "\\d".equals(node.getData())) {
        ...
        return Utils.createReDoSTree(regex);
    ...
    public static TreeNode refactorAssertPattern(TreeNode treeNode) throws InterruptedException {
        if (Constant.PATTERN_ASSERT_PATTERN.matcher(treeNode.getData()).find()) {
            Stack<TreeNode> stack = new Stack<>();
            stack.add(treeNode);
            while (!stack.isEmpty()) {
                TreeNode node = stack.pop();
                if (isGeneralizedCountingNode(node)) {
                    ...
                    if (isLookAroundNode(child)) {
                        Constant.AssertType type = getAssertType(child);
                    ...
                    Constant.AssertType type = getAssertType(node);
