package rengar.thirdparty.hunter.cn.ac.ios.TreeNode;

import rengar.thirdparty.hunter.cn.ac.ios.Bean.GroupContent;
import rengar.thirdparty.hunter.cn.ac.ios.PCRERegex.PCREBuilder;
import rengar.thirdparty.hunter.cn.ac.ios.PCRERegex.PCREParser;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.BracketUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.Constant;
import org.antlr.v4.runtime.tree.ParseTree;
import org.apache.commons.lang3.SerializationUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.FlagsUtils;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.GenMatchStringUtils;
...
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.NegateUtils.deleteZeroWidthAssertion;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.NegateUtils.removeNegateSymbol;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.RegexUtils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.UnicodeBackslashU.unicodeToCn;
...
public class TreeNode implements Iterable<TreeNode>, Serializable {
    ...
    private Constant.AssertType assertType;
...
//    }
    ...
    public void setAssertType(Constant.AssertType assertType) {
...
//        return createReDoSTree(node.getData());
        Utils.updateChainIndex(this.getRoot(), "0");
    }
    ...
    private void removeI(TreeNode treeNode) throws InterruptedException {
        ...
        if (FlagsUtils.isLocalFlagsX(data) || FlagsUtils.isLocalFlagsI(data) || FlagsUtils.isLocalFlagsS(data)) {
            return;
        }
        if (data.length() == 1) {
            ...
            // [a-z](a|b)[a-z] 特殊处理
            //  [] 中括号内部处理，不考虑中括号内部嵌套
            if (BracketUtils.isBracketsNode(treeNode) || isNegateNode(treeNode)) {
                ...
                for (TreeNode child : children) {
                    ...
                    if (child.getData().length() == 1 && Character.isLetter(child.getData().charAt(0))) {
                        ...
                        afterProcess.add(Utils.createReDoSTree("\\-"));
                    }
                    // 字母集合加入集合 如 a-z
                    else if (BracketUtils.isCollectionLetter(child.getData())) {
                ...
                if (letterSet.size() > 0) {
                    ...
                    // 添加新的字母节点
                    ArrayList<String> letters = BracketUtils.getLettersSimplyInsensitive(letterSet);
    ...
    private void removeS(TreeNode treeNode) throws InterruptedException {
        if (Constant.FLAGS_REGEXP_S_REPLACE_PATTERN.equals(treeNode.getData())) {
            return;
        }
        if (".".equals(treeNode.getData())) {
            if (!BracketUtils.isInBrackets(treeNode) && !BracketUtils.isInNegateNode(treeNode)) {
                treeNode.updateTreeByModifyNode(Constant.FLAGS_REGEXP_S_REPLACE_PATTERN);
...
//                data = "(?:" + data + "|" + data.toLowerCase() + ")";
    ...
    public void getNodeByRemoveLocalFlag() throws InterruptedException {
        if (this.data.contains(Constant.FLAGS_X_START) || this.data.contains(Constant.FLAGS_X_END)) {
            getNodeByRemoveLocalFlagX(this);
        }
        if (this.data.contains(Constant.FLAGS_I_START) || this.data.contains(Constant.FLAGS_I_END)) {
            getNodeByRemoveLocalFlagI(this);
        }
        if (this.data.contains(Constant.FLAGS_S_START) || this.data.contains(Constant.FLAGS_S_END)) {
    ...
    private TreeNode getNodeByRemoveLocalFlagX(TreeNode treeNode) {
        if (FlagsUtils.isLocalFlagsX(treeNode.getData())) {
            return null;
        }
        ...
        for (TreeNode child : children) {
            if (flag) {
                if (child.getData().equals(Constant.FLAGS_X_START)) {
                    flag = true;
                } else if (child.getData().equals(Constant.FLAGS_X_END)) {
                    flag = false;
                } else {
    ...
    private TreeNode getNodeByRemoveLocalFlagI(TreeNode treeNode) throws InterruptedException {
        if (FlagsUtils.isLocalFlagsI(treeNode.getData())) {
            return null;
        }
        List<TreeNode> children = treeNode.getChildList();
        if (children.isEmpty()) {
        ...
        for (TreeNode child : children) {
            if (flag) {
                if (child.getData().equals(Constant.FLAGS_I_START)) {
                    flag = true;
                } else if (child.getData().equals(Constant.FLAGS_I_END)) {
                    flag = false;
                } else {
    ...
    private TreeNode getNodeByRemoveLocalFlagS(TreeNode treeNode) throws InterruptedException {
        if (FlagsUtils.isLocalFlagsS(treeNode.getData())) {
            return null;
        }
        List<TreeNode> children = treeNode.getChildList();
        if (children.isEmpty()) {
        ...
        for (TreeNode child : children) {
            if (flag) {
                if (child.getData().equals(Constant.FLAGS_S_START)) {
                    flag = true;
                } else if (child.getData().equals(Constant.FLAGS_S_END)) {
                    flag = false;
                } else {
    ...
    private TreeNode removeLocalI(TreeNode treeNode) throws InterruptedException {
        String data = treeNode.getData();
        if (FlagsUtils.isLocalFlagsI(data)) {
            return null;
        }
        if (FlagsUtils.isLocalFlagsX(data)) {
            return treeNode;
        }
        if (FlagsUtils.isLocalFlagsS(data)) {
            return treeNode;
        }
        // 单字母处理 直接替换
            ...
            //  [] 中括号内部处理，不考虑中括号内部嵌套
            if (BracketUtils.isBracketsNode(treeNode)) {
                LinkedList<TreeNode> afterList = new LinkedList<>();
                Set<Character> letterSet = new LinkedHashSet<>();
                for (TreeNode child : children) {
                    // 单字母加入集合
                    if (child.getData().length() == 1 && Character.isLetter(child.getData().charAt(0))) {
                        ...
                        afterList.add(Utils.createReDoSTree("\\-"));
                    }
                    // 字母集合加入集合
                    else if (BracketUtils.isCollectionLetter(child.getData())) {
                        letterSet.addAll(BracketUtils.getLetterSet(child.getData()));
                    } else {
                        if (!FlagsUtils.isLocalFlagsI(child.getData())) {
                            afterList.add(removeLocalI(child));
                        }
                    }
                }
    ...
    private TreeNode removeLocalX(TreeNode treeNode) {
        if (" ".equals(treeNode.getData())) {
            return null;
        } else if (FlagsUtils.isLocalFlagsX(treeNode.getData())) {
            return null;
        } else if (treeNode.isLeaf() || BracketUtils.isBracketsNode(treeNode)) {
            return treeNode;
        } else {
            LinkedList<TreeNode> children = (LinkedList<TreeNode>) treeNode.getChildList();
            LinkedList<TreeNode> afterList = new LinkedList<>();
    ...
    public void transNonGreedyQuantifier() {
        if (! Constant.pattern_nonGreedyMatching.matcher(this.data).find()) {
            return;
        }
        ...
        while (!stack.isEmpty()) {
            ...
            if ((data.startsWith("*") || data.startsWith("+") || data.startsWith("?") || data.startsWith("{")) && (data.endsWith("+") || data.endsWith("?"))) {
                if (Constant.pattern_nonGreedyMatching.matcher(data).matches()) {
    ...
    public void removeBlankStr() throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            ...
            if (isOnlyEmptyNode(node)) {
                ...
                if (par == null) {
                    TreeNode newNode = Utils.createReDoSTree("");
...
//                        TreeNode child1 = node.getChild(1);
    ...
    public void insertChild(String child, int index) throws InterruptedException {
        ...
        Utils.getReDoSTreeHelper1(tree, ReDoSTree);
        TreeNode newReDoSTree = new TreeNode(child);
        Utils.getReDoSTreeHelper2(ReDoSTree, newReDoSTree);
    ...
    public void insertChild(String child) throws InterruptedException {
        ...
        TreeNode ReDoSTree = new TreeNode(child);
        ReDoSTree = Utils.getReDoSTreeHelper1(parseTree, ReDoSTree);
        TreeNode newReDoSTree = new TreeNode(child);
        Utils.getReDoSTreeHelper2(ReDoSTree, newReDoSTree);
    ...
    public void updateTreeByModifyNode(String data, boolean flag) throws InterruptedException {
        TreeNode node = Utils.createReDoSTree(data);
...
//        return letterSet;
    ...
    public Set<String> getLetterSet(boolean addDefaultw) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.isLeaf()) {
                if (Pattern.matches(Constant.COUNTING, node.getData()) || (node.getData().length() == 1 && FlagsUtils.isSpecialCharacter(node.getData().charAt(0)))) {
                    continue;
                }
                set.add(node.getData());
            } else {
                if (BracketUtils.isBracketsNode(node) || isNegateNode(node)) {
                    int i = node.getData().startsWith("[^") ? 2 : 1;
                    for (; i < node.getChildCount() - 1; i++) {
                        String childData = node.getChild(i).getData();
                        if (childData.length() == 1 && BracketUtils.isSpecialCharacterBracket(childData.charAt(0))) {
                            ...
                            if (childData.contains("-") && childData.length() > 3 && childData.length() <= 5) {
                                ...
                                childData = childData.replace("special_slash_ios_ac_cn", "\\");
                                set.add(childData);
                            } else if (BracketUtils.isCollectionSymbol(childData)) {
                                set.add(childData);
                            } else {
                    ...
                    for (TreeNode child : node.getChildList()) {
                        if (!(Pattern.matches(Constant.FLAGS_REDUCES, child.getData()))) {
                            queue.add(child);
                        }
                    }
                }
        ...
        for (String str : set) {
            if ("\\w".equals(str)) {
                sortSet.addAll(Arrays.asList(Constant.w_MATCH_CHARACTER_LIST));
            } else if ("\\d".equals(str)) {
                sortSet.addAll(Arrays.asList(Constant.d_MATCH_CHARACTER_LIST));
            } else if (".".equals(str)) {
                // . 不作处理，表示当前字母表集合
                // sortSet.addAll(Arrays.asList(dot_MATCHE_CHARACTER));
            } else if ("\\s".equals(str)) {
                sortSet.addAll(Arrays.asList(Constant.s_MATCH_CHARACTER_LIST));
            } else if (BracketUtils.isCollectionSymbol(str)) {
            ...
            } else if (Constant.COMPLEMENTARY_SET_SYMBOL.containsKey(str)) {
                sortSet.addAll(Constant.COMPLEMENTARY_SET_SYMBOL.get(str));
            } else {
                sortSet.add(str);
            }
        }
        ...
        for (String string : sortSet) {
            if (string.length() == 1 && BracketUtils.isSpecialCharacterBracket(string.charAt(0))) {
                newSet.add("\\" + string);
            } else {
                newSet.add(string);
            }
    ...
    public Set<String> getLetterSetMustHas() {
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.isLeaf()) {
                ...
                if (BracketUtils.isBracketsNode(node)) {
                    int i = node.getData().startsWith("[^") ? 2 : 1;
                    for (; i < node.getChildCount() - 1; i++) {
                        String childData = node.getChild(i).getData();
                        if (childData.length() == 1 && BracketUtils.isSpecialCharacterBracket(childData.charAt(0))) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (node.isNowNodeChildOrGrandchild(sNode)) {
                ...
                if (node.isNowNodeChildOrGrandchild(sNode)) {
                    ...
                    if (node.isLeaf() || BracketUtils.isBracketsNode(node) || isNegateNode(node)) {
    ...
    public String getMatchStr(TreeNode sNode) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (node.isNowNodeChildOrGrandchild(sNode)) {
                ...
                if (node.isLeaf() || BracketUtils.isBracketsNode(node)) {
    ...
    public String getMatchStr() {
        ...
        while (!stack.isEmpty()) {
            ...
            } else if (BracketUtils.isBracketsNode(node) || isNegateNode(node)) {    // 修改
                boolean flag = false;
                Pattern pattern = Pattern.compile(node.getData());
                for(String c: Constant.dot_MATCH_CHARACTER_LIST) {
...
//        stack.add(this);
        ...
        while (! stack.isEmpty()) {
            ...
            if (isZeroWidthAssertionNode(node)) {
                ...
                } else if (assertType == Constant.AssertType.WORD_BOUNDARY_ASSERT) { // \b
                    zeroWidthAssertionNode = node;
                } else if (assertType == Constant.AssertType.NON_WORD_BOUNDARY_ASSERT) { // \B
                    zeroWidthAssertionNode = node;
                }
                continue;
            }
            if (isGeneralizedCountingNode(node)) {
                ...
                if (zeroWidthAssertionNode != null) {
                    Constant.AssertType assertType = getAssertType(zeroWidthAssertionNode);
                    if (assertType == Constant.AssertType.POSITIVE_AHEAD || assertType == Constant.AssertType.NEGATIVE_AHEAD) {  // (?= 和 (?!
                        TreeNode treeNode = Utils.createReDoSTree(zeroWidthAssertionNode.getData() + node.getData());
                        // 使用重写后的去首尾^$
...
//                        treeNode = removeBlankStr(treeNode);
                        ...
                        List<String> regexList1 = GenMatchStringUtils.getTranslateRegexForAssertionsList(treeNode);
                        String example = GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList1, 0);
                        if (example == null) example = "";
                        s.append(example);
                        zeroWidthAssertionNode = null;
                    } else if (assertType == Constant.AssertType.POSITIVE_BEHIND || assertType == Constant.AssertType.NEGATIVE_BEHIND){   // (?<= 和 (?<!
...
//                        s.append(example);
                        ...
                        if (s.length() == 0) {
                            List<String> regexList1 = GenMatchStringUtils.getTranslateRegexForAssertionsList(node);
            ...
            if (node.isLeaf()) {
                if (isQuantifierNode(node) || (node.getData().length() == 1 && FlagsUtils.isSpecialCharacter(node.getData().charAt(0)))) {
                    continue;
                } else {
                    if (zeroWidthAssertionNode != null) {
                        Constant.AssertType assertType = getAssertType(zeroWidthAssertionNode);
                        if (assertType == Constant.AssertType.POSITIVE_AHEAD || assertType == Constant.AssertType.NEGATIVE_AHEAD) {  // (?= 和 (?!
                            TreeNode treeNode = Utils.createReDoSTree(zeroWidthAssertionNode.getData() + node.getData() + "[\\s\\S]*");
                            // 使用重写后的去首尾^$
                            treeNode.deleteCaretAndDollarSymbols();
                            // 新版重写空串
//                            treeNode = removeBlankStr(treeNode);
                            ...
                            treeNode.deleteNonCapturingGroupFlag();
                            List<String> regexList1 = GenMatchStringUtils.getTranslateRegexForAssertionsList(treeNode);
                            String example = GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList1, 1);
...
//                            s.append(example.charAt(0));
            ...
            } else if (BracketUtils.isBracketsNode(node)) {
                if (zeroWidthAssertionNode != null) {
                    Constant.AssertType assertType = getAssertType(zeroWidthAssertionNode);
                    if (assertType == Constant.AssertType.POSITIVE_AHEAD || assertType == Constant.AssertType.NEGATIVE_AHEAD) {  // (?= 和 (?!
                        TreeNode treeNode = Utils.createReDoSTree(zeroWidthAssertionNode.getData() + node.getData() + "[\\s\\S]*");
...
//                        treeNode = removeBlankStr(treeNode);
                        ...
                        TreeNode treeNode = Utils.createReDoSTree(node.getData() + zeroWidthAssertionNode.getData());
                        // 使用重写后的去首尾^$
                        treeNode.deleteCaretAndDollarSymbols();
                        // 新版重写空串
//                        treeNode = removeBlankStr(treeNode);
        ...
        if (zeroWidthAssertionNode != null) {
            Constant.AssertType assertType = getAssertType(zeroWidthAssertionNode);
            if (assertType == Constant.AssertType.WORD_BOUNDARY_ASSERT) {    // 最后一个如果是\b
                if (s.length() == 0) {
                    s.append("a");
                } else {
                    char c = s.charAt(s.length() - 1);
            ...
            } else if (assertType == Constant.AssertType.NON_WORD_BOUNDARY_ASSERT) { // 最后一个如果是\B
                if (s.length() == 0) {
        ...
        result = GenMatchStringUtils.repairLineFeedsCarriageReturnsAndTabCharacters(result);
...
//            if (isInLookAroundNode(leafNode)) continue;
            if (BracketUtils.isInLookAroundNode(leafNode)) {
            ...
            while (!tempLeafNode.equals(this)) {
                if (!BracketUtils.isInBrackets(tempLeafNode) && !BracketUtils.isInNegateNode(tempLeafNode)) {
                    if (BracketUtils.isBracketsNode(tempLeafNode) || isNegateNode(tempLeafNode)) allLeafNode2.add(tempLeafNode);
...
//        System.out.println(arrayList);
    ...
    public TreeNode refactorBracketToParentheses() throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            TreeNode node = stack.pop();
            if (BracketUtils.isBracketsNode(node)) {
                Set<String> set = BracketUtils.simplifyLetters(node.getLetterSet(false), Constant.SimplyLevel.LOW);
                StringBuilder stringBuilder = new StringBuilder("(");
                for (String str : set) {
                    if (str.length() == 3) {
                        str = "[" + str + "]";
                    ...
                    // 转义字符 需要添加 \ 才能被识别
                    if (str.length() == 1 && FlagsUtils.isSpecialCharacter(str.charAt(0))) {
                        str = "\\" + str;
                    }
                ...
                if (Constant.SLASH_SET_SYMBOL.containsKey(value)) {
                    node.setData(Constant.SLASH_SET_SYMBOL.get(value));
                    node.getChildList().clear();
                } else if (".".equals(node.getData())) {
                    Set<String> set = new HashSet<>();
                    set.add("!");
                    set.addAll(BracketUtils.simplifyLetters(letterSet, Constant.SimplyLevel.LOW));
                    StringBuilder stringBuilder = new StringBuilder("(");
                    for (String str : set) {
                        if (str.length() == 3) {
                            str = "[" + str + "]";
                    ...
                    node.setData(stringBuilder.toString());
                    node.getChildList().clear();
                } else if (node.getData().length() == 1 && BracketUtils.isSpecialCharacterNonBracket(node.getData().charAt(0))) {
        ...
        return Utils.createReDoSTree(getLeafsData());
    }
    ...
    public void escapeHexadecimal() throws InterruptedException {
        Matcher matcher_1 = Constant.PATTERN_ESCAPE_HEXADECIMAL_REGEX_1.matcher(this.data);
        Matcher matcher_2 = Constant.PATTERN_ESCAPE_HEXADECIMAL_REGEX_2.matcher(this.data);
        Matcher matcher_3 = Constant.PATTERN_ESCAPE_HEXADECIMAL_REGEX_3.matcher(this.data);
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.isLeaf()) {
                if (nodeData.matches(Constant.ESCAPE_HEXADECIMAL_REGEX_1)) {
...
//                    }
                ...
                if (nodeData.matches(Constant.ESCAPE_HEXADECIMAL_REGEX_2)) {
...
//                    }
            ...
            } else if (nodeData.matches(Constant.ESCAPE_HEXADECIMAL_REGEX_3)) {
...
//			System.out.println(root.data);
    ...
    public boolean isConnectorInBracketNode() {
        if (!BracketUtils.isInBrackets(this) || !this.getData().equals("-") || this.getParent() == null) {
            return false;
        }
        if (this.getParent().getChildCount() == 3) {
    ...
    public int getFlexibleN() {
        ...
        while (!node.isRoot()) {
            ...
            node = node.getParent();
            // 如果是连接结点
            if (!node.isRoot() && !BracketUtils.isBracketsNode(node) && !isNegateNode(node) && !isZeroWidthAssertionNode(node) && !node.isLeaf() && !isGroupNode(node) && !isGeneralizedCountingNode(node) && !isOrNode(node)) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (BracketUtils.isBracketsNode(node) || isNegateNode(node)) {
                ...
                for (; i < node.getChildCount() - 1; i++) {
                    ...
                    } else if (BracketUtils.isGeneralizedCollectionSymbol(node.getChild(i))) {
...
//                }
                ...
                if ((BracketUtils.isInBrackets(node) || BracketUtils.isInNegateNode(node)) &&
                    ...
                    node.setLast(new HashSet<>());  // 置为无效值∅
                    continue;
                } else if (Pattern.matches(Constant.ALL_COUNTING, node.getData())) {
                    node.setNullable(-1);   // 置为无效值
                    node.setFirst(new HashSet<>()); // 置为无效值∅
                    node.setLast(new HashSet<>());  // 置为无效值∅
                    continue;
                    ...
                    if (node.getData().equals("\\w")) {
                        Set<String> auxiliarySet = new HashSet<>(Arrays.asList(Constant.w_MATCH_CHARACTER_LIST));
                        node.setFirst(auxiliarySet);
                        node.setLast(auxiliarySet);
                    } else if (node.getData().equals("\\d")) {
                        Set<String> auxiliarySet = new HashSet<>(Arrays.asList(Constant.d_MATCH_CHARACTER_LIST));
                        node.setFirst(auxiliarySet);
                        node.setLast(auxiliarySet);
...
//                    node.setLast(dot_MATCH_CHARACTER);
                        ...
                        Set<String> auxiliarySet = new HashSet<>(Arrays.asList(Constant.s_MATCH_CHARACTER_LIST));
                        Set<String> differenceSet = new HashSet<>(rootAlphabet);
                        differenceSet.removeAll(auxiliarySet);                 // 用Σ减并集
                        node.setFirst(differenceSet);
                        node.setLast(differenceSet);
...
//                System.out.println(node.getData() + " " + isZeroNode(node));
            ...
            if (isOrNode(node)) {
                ...
                if ((BracketUtils.isInBrackets(node) || BracketUtils.isInNegateNode(node)) && BracketUtils.isGeneralizedCollectionSymbol(node)) {
...
//                node.setFollowLast(differenceSet);
            ...
            if (isOrNode(node)) {
                ...
                if (BracketUtils.isInBrackets(node)) {
                    node.setdFlexible(1);       // 按普通结点对待
                    node.setbFlexible(false);   // 按普通结点对待
                    node.setFollowLast(new HashSet<>());    // 按普通结点对待
                    if (node.getData().equals("[") && node.isFirstChild()) {
    ...
    public void rewriteBackReferences() throws InterruptedException {
        ...
        groupContentChainIndexList.add(new GroupContent(Utils.createReDoSTree("-"), "", ".", "."));
        TreeNode tree = this;
        Stack<TreeNode> stack = new Stack<>();
        stack.add(tree);
        boolean flag = false;
        while (!stack.isEmpty()) {
            tree = stack.pop();
            if (BracketUtils.isBracketsNode(tree)) {
                continue;
            }
            if (isGroupNode(tree)) {
...
//                }
                // 再次修正 不应该将出现在[]内的^$删除
                data = child.getData();
                if ((data.equals("^") || data.equals("$") || data.equals("\\A") || data.equals("\\Z") || data.equals("\\z")) && !BracketUtils.isInBrackets(child) && !BracketUtils.isInNegateNode(child)) {
//                    chainIndexList.add(child.getChainIndex());
...
//        }
    ...
    public void addBackslashBeforeSomeCharacters() throws InterruptedException {
        ...
        for (TreeNode leafNode : allLeafNodes) {
            if (BracketUtils.isSomeCharactersNeedAddABackslashBeforeIt(leafNode.getData())) {
    ...
    public void rewriteSpecialBackslashCharacterForDifferentLanguage (String language) throws InterruptedException {
        ...
        if (language.equals("java")) {
            hashMap = Constant.SPECIAL_BACKSLASH_CHARACTER_FOR_JAVA;
        } else if (language.equals("python")) {
            hashMap = Constant.SPECIAL_BACKSLASH_CHARACTER_FOR_PYTHON;
        } else if (language.equals("javascript")) {
            hashMap = Constant.SPECIAL_BACKSLASH_CHARACTER_FOR_JAVASCRIPT;
        } else {
            return;
        }
    ...
    public void rewriteUnicodeNumberInBracketNode() throws InterruptedException {
        ...
        while (!queue.isEmpty()) {
            ...
            if (BracketUtils.isBracketsNode(node) || isNegateNode(node)) {
                for (int i = 0; i < node.getChildCount(); i++) {
                    String data = node.getChild(i).getData();
                    if (BracketUtils.isGeneralizedCollectionSymbol(node.getChild(i))) {
                        ...
                        if (data.startsWith("\\") && data.length() > 1) {
                            ...
                            if (num != -1) {
                                ...
                                // 把当前的值转换了
                                String newData = Constant.OCTAL_TO_HEX.get("\\" + num);
                                if (newData != null)
    ...
    private boolean needDealWithCharacterClassInJava() {
        ...
        while (!stack.isEmpty()) {
            TreeNode node = stack.pop();
            if (BracketUtils.isBracketsNode(node) || isNegateNode(node) || BracketUtils.isIntersectionNode(node)) {
                return true;
            }
            for (int i = node.getChildCount() - 1; i >= 0; i--) {
                stack.push(node.getChild(i));
...
//                        System.out.println(Integer.toHexString(i) + " " + isMatch);
    ...
    public void dealWithIntersectOperationInJava() throws InterruptedException {
        ...
        while (!stack.isEmpty()) {
            ...
            if (BracketUtils.isBracketsNode(node) || isNegateNode(node)) {
                ...
                if (BracketUtils.isIntersectionNode(node)) {
                    ...
                    for (int i = 0; i < node.getChildCount(); i += 3) {
                        ...
                        } else if (BracketUtils.isBracketsNode(node.getChild(i))) {
                            positiveNodeList.add(node.getChild(i));
                        } else {
                            positiveNodeList.add(Utils.createReDoSTree("[" + node.getChild(i).getData() + "]"));
                        }
                    }
    ...
    public static void main(String[] args) throws InterruptedException {
        String regex = "a*b+";
        TreeNode treeNode = Utils.createReDoSTree(regex);
