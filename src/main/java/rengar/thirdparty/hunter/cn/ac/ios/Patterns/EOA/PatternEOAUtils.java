package rengar.thirdparty.hunter.cn.ac.ios.Patterns.EOA;
...
import rengar.thirdparty.hunter.cn.ac.ios.Bean.*;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.*;
import rengar.thirdparty.hunter.dk.brics.automaton.Automaton;

import java.util.*;

import static rengar.thirdparty.hunter.cn.ac.ios.TreeNode.Utils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.NegateUtils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.RegexUtils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.SpecialStringUtils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.SplitRegexUtils.*;
...
public class PatternEOAUtils {
    private static TreeNode getReDoSTree(String regex, String language) throws InterruptedException {
        ...
        regex = FlagsUtils.reduceLocalFlags(regex);
        regex = FlagsUtils.removeAnnotationByFlagX(regex);
        regex = FlagsUtils.processLocalFlag(regex);
        regex = FlagsUtils.replaceLocalFlagGM(regex);
...
//        regex2 = repairLineFeedsCarriageReturnsAndTabCharacters(regex2);
        ...
        Automaton automaton = DkBricsAutomatonUtils.getIntersectionAutomaton(regexList1, null, 0);
        if (DkBricsAutomatonUtils.isAcceptEmptyString(automaton)) return 0;

        // .*不转义的
        List<String> regexList2 = new ArrayList<>();
        regexList2.add(firstCharacter + ".*");
        String str = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1, regexList2, 0);
    ...
    private static Set<String> getInfixSetForEOA(String r0, String r1, String r2, String r3, String r4, List<Integer> candidateCountingList, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {  // 考虑零宽断言
            ...
            List<String> r0Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r0Tree);
            List<String> r1Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r1Tree);
            List<String> r2Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r2Tree);
            List<String> r3Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r3Tree);
            List<String> r4Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r4Tree);
            ...
            infix1 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r0Trans, 0);
            ...
            for (int counting: candidateCountingList) {
                infix2 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r1AndR3Trans, counting);
                if (infix2 != null) infix2Set.add(infix2);
            }
            if (infix2Set.isEmpty()) return null;

            infix3 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r2Trans, 0);
            if (infix3 == null) return null;

            infix4 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r4Trans, 0);
            ...
            String infix1 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r0)), null, 0);
            if (infix1 == null) return null;


            Set<String> infix2Set = new HashSet<>();
            for (int counting: candidateCountingList) {
                String infix2 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Arrays.asList(r1, r3)), null, counting);
                if (infix2 != null) infix2Set.add(infix2);
            }
            if (infix2Set.isEmpty()) return null;

            String infix3 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r2)), null, 0);
            if (infix3 == null) return null;

            String infix4 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r4)), null, 0);
    ...
    private static String getInfixForEOA(String r0, String r1, String r2, String r3, String r4, int counting, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {  // 考虑零宽断言
            ...
            List<String> r3Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r3Tree);
            List<String> r4Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r4Tree);
            String infix1 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r0Trans, 0);
            if (infix1 == null) return null;

            List<String> r1AndR3Trans = new ArrayList<>();
            r1AndR3Trans.addAll(r1Trans);
            r1AndR3Trans.addAll(r3Trans);
            String infix2 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r1AndR3Trans, counting);
            if (infix2 == null) return null;

            String infix3 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r2Trans, 0);
            if (infix3 == null) return null;

            String infix4 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r4Trans, 0);
...
//        if (beta1Nullable == 1 && beta2Nullable == 1) return "";
        ...
        if (considerZeroWidthAssertions) {
            ...
            List<String> beta1Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(beta1Tree);
            List<String> beta2Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(beta2Tree);
            List<String> regexList = new ArrayList<>();
            regexList.addAll(beta1Trans);
            regexList.addAll(beta2Trans);
            String infix = GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList, 0);
            ...
            String infix = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1);
            return infix != null ? infix : "";
        }
...
//        if (beta1Nullable == 1 || beta2Nullable == 1) return "";
        ...
        if (considerZeroWidthAssertions) {
            ...
            regexList.addAll(beta1Trans);
            regexList.addAll(beta2Trans);
            String infix = GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList, 1);
            ...
            for (int i = 0; i < allPossibleChildren.size(); i++) {
                ...
                if (isGeneralizedCountingNode(candidateNode)) {
                    ...
                    if (!r0.equals("")) {
                        ...
                        getTranslateRegexForAssertionsListHelper(r0Tree);
                        List<String> r0Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r0Tree);
                        infix1 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r0Trans, 1);
                    ...
                    if (!r4.equals("")) {
                        ...
                        getTranslateRegexForAssertionsListHelper(r4Tree);
                        List<String> r4Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(r4Tree);
                        infix2 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r4Trans, 1);
...
//        System.out.println("infix = " + infix);
            ...
            for (int i = 0; i < allPossibleChildren.size(); i++) {
                ...
                if (isGeneralizedCountingNode(candidateNode)) {
                    ...
                    String infix1 = "";
                    if (!r0.equals("")) {
                        infix1 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Arrays.asList("(" + r0 + ")+", candidateNode.getData())));
                        if (infix1 == null) continue;
                    }
                    String r4 = getR4(betaTree, candidateNode);            // 第二个counting后面的正则
                    String infix2 = "";
                    if (!r4.equals("")) {
                        infix2 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(new ArrayList<>(Arrays.asList("(" + r4 + ")+", candidateNode.getData())));
                        if (infix2 == null) continue;
                    }
                    return infix1 + infix2;
                }
    ...
    private static ReDoSBean getEOAReDoSBeanHelper(TreeNode sourceReDoSTree, TreeNode standardizedReDoSTree, String regex)  {
        ...
        try {
            ...
            while (!stack.isEmpty()) {
                ...
                if (BracketUtils.isInBrackets(node) || BracketUtils.isInNegateNode(node)) continue;
    ...
    //                System.out.println(allPossibleChildren);
                    ...
                    for (int i = 0; i < allPossibleChildren.size() - 1; i++) {
                        for (int j = i + 1; j < allPossibleChildren.size(); j++) {
                            ...
                            // 最近的爹如果不是连接结点 则不是EOA
                            if (!(!isOrNode(nearestParent) && !BracketUtils.isBracketsNode(nearestParent) && !isNegateNode(nearestParent) && !nearestParent.isLeaf() && !isGeneralizedCountingNode(nearestParent) && nearestParent.getChildCount() >= 2)) {    // 不是是连接结点
                                continue;
                            }
...
//                                        || (!Collections.disjoint(beta2LastSet, beta1FirstSet) && acceptEmptyString(rList.get(rList.size() - 1)))
                            ...
                            String prefix1 = DkBricsAutomatonUtils.getPrefixString(standardizedReDoSTree, node, true); // 获取的是前缀正则+中缀正则的生成串
                            String prefix2 = DkBricsAutomatonUtils.getPrefixString(standardizedReDoSTree, node, false);
...
//                            if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                            ...
                            for (String infix: infixSet) {
                                suffixSet.addAll(DkBricsAutomatonUtils.getSuffixString(standardizedReDoSTree, node, infix));
                            }
...
//                                        attackBean.setPatternType(PatternType.EOA_i_or_ii);
                    ...
                    if (! isOrNode(node) && ! isGeneralizedCountingNode(node) && ! BracketUtils.isBracketsNode(node) && ! isNegateNode(node) && ! node.isLeaf() && ! isGroupNode(node) && node.getChildCount() >= 2) {
