...
import rengar.thirdparty.hunter.cn.ac.ios.Patterns.NQ.PatternNQUtils;
...
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.DkBricsAutomatonUtils.getExampleByDkBricsAutomaton;
...
public class SpecialStringUtils {
    ...
    public static String getMatchStringWithEndLineInOrNode(TreeNode treeNode) {
        ...
        while (!stack.isEmpty()) {
            ...
            node = RegexUtils.getGroupSubNode(node);
            if (RegexUtils.isZeroWidthAssertionNode(node)) {
                continue;
            }
            if (RegexUtils.isOrNode(node)) {
                node = RegexUtils.getGroupSubNode(node);
                boolean flag = true;
                for (int i = 0; i < node.getChildCount(); i += 2) {
                    if (!RegexUtils.isZeroWidthAssertionNode(node.getChild(i))) {
            ...
            if (RegexUtils.isGeneralizedCountingNode(node)) {
                node = RegexUtils.getGroupSubNode(node);
                int num = RegexUtils.getCountingFirstNum(node.getChild(1).getData());
//                num = num == 0 ? 1 : num;
            ...
            if (node.isLeaf()) {
                ...
                s.append(RegexUtils.getMatchLeafNode(node));
            } else if (BracketUtils.isBracketsNode(node) || RegexUtils.isNegateNode(node)) {
                boolean flag = false;
...
//        return sb.toString();
    ...
    public static List<String> getSpecialSuffixStringWithNegativeAssertNode(TreeNode node) throws InterruptedException {
        ...
        for (TreeNode negativeAssertNode: allNegativeAssertNode) {
            String r0 = SplitRegexUtils.getR0(node, negativeAssertNode);
            if (RegexUtils.isNegativeLookAheadNode(negativeAssertNode)) {
                suffixList.add(getMatchString(Utils.createReDoSTree(r0)) + getMatchString(negativeAssertNode.getChild(3)));
            } else if (RegexUtils.isNegativeLookBehindNode(negativeAssertNode)) {
                suffixList.add(getMatchString(Utils.createReDoSTree(r0)) + getMatchString(negativeAssertNode.getChild(4)));
            }
        }
        return suffixList;
    }
