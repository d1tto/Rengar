package rengar.thirdparty.hunter.cn.ac.ios.Patterns.EOD;
...
import rengar.thirdparty.hunter.cn.ac.ios.Bean.AttackType;
import rengar.thirdparty.hunter.cn.ac.ios.Bean.PatternType;
import rengar.thirdparty.hunter.cn.ac.ios.Bean.ReDoSBean;
import rengar.thirdparty.hunter.cn.ac.ios.Utils.*;

import java.util.*;

import static rengar.thirdparty.hunter.cn.ac.ios.TreeNode.Utils.*;
import static rengar.thirdparty.hunter.cn.ac.ios.TreeNode.Utils.setInitialChainIndex;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.BracketUtils.isSpecialCharacterBracket;
...
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.SplitRegexUtils.getR0;
import static rengar.thirdparty.hunter.cn.ac.ios.Utils.SplitRegexUtils.getR4;
    ...
    private static String generateInfixStringForEOD(String regex1, String regex2, int model, String firstCharacter, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {
            if (model == 1) {
                ...
                List<String> treeNode1Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(treeNode1);
                List<String> treeNode2Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(treeNode2);
                ...
                return GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList, 0);
            } else if (model == 2) {
                TreeNode treeNode1 = createReDoSTree("(" + regex1 + ")+(" + regex2 + ")+");
            ...
            if (model == 1) {
                ...
                return DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1, regexList2, 0);
            } else if (model == 2) {
                List<String> regexList1 = new ArrayList<>();
                regexList1.add("(" + regex1 + ")+(" + regex2 + ")+");
            ...
            while (!stack.isEmpty()) {
                ...
                if (isGeneralizedCountingNodeWithMaxNumGreaterThanOne(node)) {
                    ...
                    for (TreeNode orNode : allOrNodeList) {
                        ...
                        String infix3 = GenMatchStringUtils.getExampleByDkBricsAutomaton2(r4Trans, 0);
                        if (infix3 != null) infix3Set.add(infix3);

                        r0 = getRegexWithoutZeroWidthAssertion(r0);
                        r4 = getRegexWithoutZeroWidthAssertion(r4);

                        infix1 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(Collections.singletonList(r0), null, 0);
                        if (infix1 != null) infix1Set.add(infix1);
                        infix3 = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(Collections.singletonList(r4), null, 0);
                        if (infix3 != null) infix3Set.add(infix3);
                            ...
                            for (int j = i + 2; j < orNode.getChildCount(); j++) {
                                ...
                                if (!Collections.disjoint(orNode.getChild(i).getFirst(), orNode.getChild(j).getFirst())) {
                                    ...
                                    if (firstCharacter.equals("\\t")) {
                                        ...
                                        firstCharacter = "\r";
                                    } else if (BracketUtils.isSpecialCharacterBracket(firstCharacter)) {
...
//                                    if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                                    ...
                                    for (String fullInfix : fullInfixSet) {
                                        suffixSet.addAll(DkBricsAutomatonUtils.getSuffixString(standardizedReDoSTree, countingNode, fullInfix));
                                    }
