package rengar.thirdparty.hunter.cn.ac.ios.Patterns.POA;
...
//    }
    ...
    private static String generateInfixStringForPOA(TreeNode root, TreeNode parent, TreeNode child, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {  // 考虑零宽断言
            List<String> innerTrans = GenMatchStringUtils.getTranslateRegexForAssertionsList(parent);
...
//            deleteZeroWidthAssertion(childCopy);
            ...
            return GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList1, 1);
        } else {
...
//        deleteZeroWidthAssertion(regexTree);
    ...
    private static Set<String> getInfixForPOA(String regex1, String regex2, String regex3, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {
            ...
            List<String> regex1Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(regex1Tree);
            List<String> regex1Regex2Regex3Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(regex1Regex2Regex3Tree);
            List<String> regex3Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(regex3Tree);
            List<String> regex1Regex3Trans = GenMatchStringUtils.getTranslateRegexForAssertionsList(regex1Regex3Tree);
            ...
            regexList1.addAll(regex1Regex2Regex3Trans);
            example = GenMatchStringUtils.getExampleByDkBricsAutomaton2(regexList1, 0);
            if (example != null && !example.equals("")) infixSet.add(example);
            ...
            String example = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1);
            if (example != null && !example.equals("")) infixSet.add(example);

            regexList1 = new ArrayList<>();
            regexList1.add("(" + regex1 + ")(" + regex3 + ")");
            regexList1.add("(" + regex1 + ")(" + regex2 + ")(" + regex3 + ")");
            example = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1);
            if (example != null && !example.equals("")) infixSet.add(example);
...
//        System.out.println("regex1 = " + regex1 + " regex2 = " + regex2 + " regex3 = " + regex3);
    ...
    private static boolean isPOAnotEOA(String r1, String r3, String r4) throws InterruptedException {
        ...
        regexList1.add(r4);
        regexList1.add(r1);
        String example = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1, null, 0);
        if (example != null) {
            return false;
        }
        ...
        regexList1.add(r3);
        example = DkBricsAutomatonUtils.getExampleByDkBricsAutomaton(regexList1, null, 0);
        if (example != null) {
            return false;
        }
...
//        List<TreeNode> allPossibleChildren = root.getAllPossibleChildren();
            ...
            for (int i = 0; i < allPossibleChildren.size(); i++) {
                for (int j = i + 1; j < allPossibleChildren.size(); j++) {
                    ...
                    // 最近的爹如果不是连接结点 则不是EOA
                    if (!(!isOrNode(nearestParent) && !BracketUtils.isInBrackets(nearestParent) && !BracketUtils.isInNegateNode(nearestParent) && !nearestParent.isLeaf() && !isGeneralizedCountingNode(nearestParent) && nearestParent.getChildCount() >= 2)) {    // 不是是连接结点
                        continue;
                    }
...
//                        infixSet.addAll(generateInfixStringForPOA(beta1StringBuilder.toString(), r2, child2.getData()));
                        ...
                        if (!infixSet.isEmpty()) {
                            ...
                            String prefix1 = DkBricsAutomatonUtils.getPrefixString(standardizedReDoSTree, child1, true); // 获取的是前缀正则+中缀正则的生成串
                            String prefix2 = DkBricsAutomatonUtils.getPrefixString(standardizedReDoSTree, child1, false);
...
//                            if (specialSuffix2 != null) suffixSet.add(specialSuffix2);
                            for (String infix : infixSet) {
                                suffixSet.addAll(DkBricsAutomatonUtils.getSuffixString(standardizedReDoSTree, child2, infix));
                            }
...
//                                infixSet = generateInfixStringForPOA(beta1StringBuilder.toString(), r2, child2.getData());
                                ...
                                if (!infixSet.isEmpty()) {
                                    ...
                                    String prefix1 = DkBricsAutomatonUtils.getPrefixString(r0Tree, childList.get(k), true); // 获取的是前缀正则+中缀正则的生成串
                                    String prefix2 = DkBricsAutomatonUtils.getPrefixString(r0Tree, childList.get(k), false);
